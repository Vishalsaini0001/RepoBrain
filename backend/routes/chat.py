...
async def ask_question(data: ChatMessage, current_user=Depends(get_current_user), db: AsyncIOMotorClient = Depends(get_db)):
...
async def clear_chat_history(repo_id: str, current_user=Depends(get_current_user), db: AsyncIOMotorClient = Depends(get_db)):
    user_id = str(current_user["_id"])
