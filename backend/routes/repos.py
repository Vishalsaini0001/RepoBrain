...
async def list_repos(current_user=Depends(get_current_user), db: AsyncIOMotorClient = Depends(get_db)):
    user_id = str(current_user["_id"])
...
async def get_repo(repo_id: str, current_user=Depends(get_current_user), db: AsyncIOMotorClient = Depends(get_db)):
    user_id = str(current_user["_id"])
...
async def delete_repo(repo_id: str, current_user=Depends(get_current_user), db: AsyncIOMotorClient = Depends(get_db)):
    user_id = str(current_user["_id"])
